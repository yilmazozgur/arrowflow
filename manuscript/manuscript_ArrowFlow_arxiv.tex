...
\begin{equation}
D_j = \norm{m_j}_q, \quad q \in \{1, 2\} \ \text{(with an optional $\ell_0$-style count, $q=0$, not a true norm)}.
\label{eq:distance}
\end{equation}
...
If $\pi_x$ omits items present in $r_j$, missing items are assigned the central position $mV$ (with $m=0.5$), and a deletion-cost weight $w=0.5$ scales their contribution to this partial-input distance (the weight enters the distance term, not the filter-update vote); an alternative variant instead displaces missing items to the end of the list and charges a deletion penalty. We use the central-imputation rule throughout unless stated otherwise. (The masking experiments of Section~\ref{sec:missing} impute by column mean \emph{before} encoding, producing complete permutations, so they do not exercise this partial-input path; it applies when vocabulary items are genuinely absent from an input.) If $r_j$ has a restricted receptive field (subset of items), the average motion is subtracted to promote translation invariance---analogous to zero-centering in convolutional layers.
...
\end{tabular}
\end{center}
An item genuinely absent from an input is assigned the central position $mV$ with $m=0.5$, so it contributes a moderate, bounded displacement rather than an extreme one, and a deletion-cost weight $w=0.5$ scales its contribution to the partial-input distance (Section~\ref{sec:ranking-layer}).
...
\textit{Upper bound.}  Let $\phi_{(2)}$ be the second-nearest filter.  Choosing $\pi' = \phi_{(2)}$ makes $d_F(\pi', \phi_{(2)}) = 0$; since the filters are distinct, no other filter lies at distance $0$, so $\phi_{(2)}$ becomes the strictly nearest filter, and as the original top filter was $\phi_{(1)} \neq \phi_{(2)}$, the output ranking changes.  Therefore $\Delta^*(\pi) \le d_F(\pi, \phi_{(2)}) = D_{(2)}(\pi)$.
\end{proof}
...
\item \textbf{(Explicit witness.)} The violation is concrete, not merely abstract: there is an explicit two-voter profile pair, preserving every voter's $a$-vs-$b$ order while only relocating the irrelevant alternative $c$, on which the consensus $a$-vs-$b$ order reverses.
\end{enumerate}
\end{proposition}
...
\]
Every voter's $a$-vs-$b$ order is identical in $\Pi$ and $\Pi'$ (voter~1 places $a$ before $b$; voter~2 places $b$ before $a$); the profiles differ only in where the irrelevant alternative $c$ is placed (which shifts the absolute rank of $a$ or $b$ within a ballot, but never their relative order).  The summed positions $(\sum_t\pi_t(a),\sum_t\pi_t(b),\sum_t\pi_t(c))$ are $(4,3,5)$ for $\Pi$ and $(3,4,5)$ for $\Pi'$, so $A(\Pi)$ ranks $b$ before $a$ while $A(\Pi')$ ranks $a$ before $b$: the relative order reverses despite the preserved pairwise preferences, exhibiting an IIA violation directly.  (A broader search over random $V=6$ profiles finds such reversals abundantly.)
\end{proof}
...
\begin{table}[ht]
\centering
\caption{Cumulative error reduction (\%) as components are added. Each row reports the best configuration at that stage drawn from the same sweep that produces Table~\ref{tab:uci-main}; the rows isolate the marginal contribution of each component rather than coming from a single end-to-end ablation run.}
...
GBM     & 2.8 & 2.8  & 6.7  & 12.2 & 18.3 & 36.7 \\
...
GBM     & 4.2  & 4.7  & 7.2  & 12.9 & 30.5 & 58.4 \\
...
\toprule
Dataset & pol\_deg & AF & SVM & RF & MLP & GBM & KNN & Avg BL \\
\midrule
...
\begin{table}[ht]
\centering
\caption{Privacy via rank transform: error on raw vs.\ ranked features. $\Delta$ = degradation in pp (negative = improvement). Bold marks ArrowFlow's $\Delta$ in each block.}
\label{tab:privacy}
\small
\begin{tabular}{llccc}
\toprule
...
\midrule
\multirow{4}{*}{Iris} & ArrowFlow & 4.4 & 2.2 & \textbf{$-$2.2} \\
 & RF & 3.3 & 3.3 & +0.0 \\
 & SVM-RBF & 3.3 & 3.3 & +0.0 \\
 & KNN & 3.3 & 6.7 & +3.3 \\
\midrule
...
\multirow{4}{*}{Digits} & ArrowFlow & 4.8 & 5.3 & \textbf{+0.5} \\
 ...
 & RF & 2.8 & 3.9 & +1.1 \\
\midrule
\multirow{3}{*}{Vehicle} & ArrowFlow & 18.4 & 22.6 & \textbf{+4.1} \\
 & MLP & 12.3 & 22.4 & +10.0 \\
...
\toprule
Dataset & Feat & AF & RF & SVM & MLP & GBM & Avg BL \\
\midrule
...
On Iris at 50\% masking (2 of 4 features zeroed), ArrowFlow achieves \textbf{24.7\%---beating all baselines} (29.3--37.3\%). On Digits, ArrowFlow degrades only 18.9pp vs.\ RF (31.9pp) and GBM (34.7pp). The advantage is strongest at dimensional extremes (very low or very high) and weakest in the mid-range where polynomial expansion amplifies the imputation effect.
...
GBM   & 62.8 & 56.1 & 28.9 & 19.2 & 3.6 \\
KNN       & 26.9 & 22.5 & 14.4 & 14.4 & 3.3 \\
\bottomrule
\end{tabular}
\end{table}

On Breast Cancer ($N$=100--200), ArrowFlow achieves 3.5--4.7\%---beating RF (8.8\%), GBM (7.9--11.4\%), and matching SVM (3.5\%). The ordinal inductive bias compensates for limited data when ordinal structure is informative. However, ArrowFlow does \emph{not} have a universal small-$N$ advantage: on Digits at $N$=20, it scores 58.1\% (second worst) while KNN achieves 26.9\%.
...
\textbf{Dataset:} the SUSHI preference dataset \citep{kamishima2003sushi}---5000 users ranking 10 sushi types. The task is a binary East/West Japan region label, derived by thresholding each user's prefecture-of-residence field (a proxy for the geographic split rather than the dataset's own region attribute). ArrowFlow hyperparameters are tuned in two phases: a first-phase screen over 2016 nominal grid points (of which only $\sim$504 are distinct, since the embedding-dimension axis is inert for natively-encoded rankings) at a single view and seed, followed by re-evaluation of the top configurations at 7 views and 5 seeds.
...
GBM             & numeric    & 35.9\% \\
KNN                 & numeric    & 37.6\% \\
ArrowFlow (native)  & native     & 37.9 $\pm$ 0.4\% \\
MLP                 & numeric    & 38.7\% \\
...
\textbf{Dataset.} UCI Gene Expression Cancer RNA-Seq \citep{dua2019uci}, derived from the TCGA Pan-Cancer project \citep{weinstein2013cancer}: 801 samples, 20{,}531 genes, 5 cancer types (BRCA, KIRC, COAD, LUAD, PRAD). We select the top $K$ genes by mutual information, rank-transform each sample to obtain a permutation of $K$ items, and split 80/20 stratified.
...
GBM    & ranked   & 4.3  & \textbf{0.0}  & \textbf{0.0} \\
\midrule
...
\paragraph{Honest assessment.} ArrowFlow native and SVM on rank-transformed data provide \emph{comparable} batch-effect robustness---both benefit from the rank transform, and SVM-ranked is marginally more accurate across the realistic operating range ($\sigma \le 1.0$); only at the extreme $\sigma = 1.5$ do the rank methods separate appreciably (SVM-ranked $43.5\%$, RF-ranked $34.2\%$, ArrowFlow native $60.8\%$), where all are well outside any realistic batch-effect regime. The robustness advantage belongs to the \emph{ordinal representation principle}, not to ArrowFlow specifically. However, ArrowFlow embodies this principle architecturally: it \emph{cannot} use magnitude information even if present, making batch-effect invariance a structural guarantee rather than a preprocessing choice that could be accidentally omitted. For clinical genomics pipelines, where data from heterogeneous sources must be classified reliably, this architectural guarantee has engineering value.

\subsection{MNIST via PCA: Isolating the Sort-Layer Classifier}
\label{sec:mnist-pca}

\textbf{Rationale.} The preceding experiments test ArrowFlow on small tabular datasets (150--2310 samples) and domain-specific data. Two questions remain: (1)~does ArrowFlow scale to a larger, harder image classification task? (2)~how much of ArrowFlow's performance comes from the sort-layer \emph{classifier} versus the argsort \emph{encoding}? To answer both, we give ArrowFlow and baselines the \emph{exact same} PCA-reduced MNIST features---baselines receive PCA vectors directly, while ArrowFlow receives them through projection $\to$ argsort. Any accuracy difference is purely due to ArrowFlow's ordinal learning versus conventional classifiers on identical information.

\textbf{Setup.} MNIST \citep{lecun1998mnist} (60{,}000 train / 10{,}000 test, 10 classes). Pipeline: 784 pixels $\to$ StandardScaler $\to$ PCA($n$, whitened) $\to$ baselines or ArrowFlow. PCA dimensions: $\{16, 32, 64\}$; training sizes: $\{1{,}000, 5{,}000, 10{,}000\}$. Baselines use GridSearchCV (RF, SVM-RBF, MLP, KNN, GBM). ArrowFlow uses 7-view ensemble with diverse projections; architectures sweep width $\{128, 256, 512, 1024\}$ and depth $\{2, 3\}$ layers. ArrowFlow results are mean $\pm$ SE over 3 simulations. For computational tractability the baseline GridSearchCV grids are reduced and use $\mathrm{cv}=2$ at the largest training sizes.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Slow convergence:} The permutation-matrix accumulation makes conservative updates, requiring 100--400 iterations vs.\ $\sim$50 epochs for gradient-based methods.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Text and sequence domains:} A word-presence (count-vectorizer) encoding that converts documents to native permutations opens a path for applying ArrowFlow to text classification without the projection pipeline.
...
\begin{enumerate}[leftmargin=2em]
  \item ArrowFlow \textbf{reaches parity with the best tuned baselines on Iris} (2.7\% vs.\ 3.3\%; a sub-sample margin, see Section~\ref{sec:experiments}) and is competitive on 5 of 7 UCI datasets, showing that zero-floating-point-parameter architectures can rival gradient-trained networks.
  ...
  \item On \textbf{MNIST via PCA}, ArrowFlow achieves 9.1\% error using only ordinal comparisons. It \textbf{scales strongly} with width and depth (18.7\%$\to$9.1\%), and the scaling curve has not plateaued, suggesting limited capacity is at least one bottleneck (though the learning rule may also limit attainable accuracy).
